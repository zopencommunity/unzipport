...
unzip=$(echo $(/bin/type unzip) | cut -f3 -d ' ')
...
test_1 () {
 ...
 [ -e "$TEST_DIR/archive.zip" ] && echo "dh exists -$TEST_DIR/archive.zip" 
  echo "dh filename = $filename" 
  echo "dh test dir = $TEST_DIR"
 unzipfile="$TEST_DIR/archive.zip" 
  $unzip -d $TEST_DIR $unzipfile 
  [ $? -ne 0 ] && {
    echo "Unzip failed."
    FAILED=$[ $FAILED+1 ]
    return 1
...
test_2()
{
  echo "dh dtest_dir = $DTEST_DIR, filename = $filename"
  [ ! -f "$TEST_DIR/$filename" ] && {
     echo "Unzipped archive doesn't contain archived file."
     FAILED=$[ $FAILED+1 ]
     return 1
  }
  PASSED=$[ $PASSED+1 ]
...
test_3()
{
  cmp -s "$TEST_DIR/$filename" "$filename" || {
    echo "Unzipped file is different!"
    FAILED=$[ $FAILED+1 ]
    return 1
  }
